...
## 5bm. 中文适配第一阶段：另一张前页，同样的保留（2026-08-17）

`README.zh-CN.md` 落地，两页顶部互相跳转。翻译是一个谨慎的项目悄悄不再谨慎的地方：
英文前页每个词都写得能兑现——尚未上 PyPI、一家银行、一个平台、其余全打叉——
而读中文那页的人永远看不到英文那页。**换一种语言不会让一句话变真**，所以第二张前页
从第一次提交起就带着和第一张一样的闸门。

`tests/test_readme_translations.py` 扫 `README.md` 与它旁边的每一个 `README.<lang>.md`，
所以一年后加日文的人不需要先读这个文件就自动继承全部检查。四条：

1. **互相能到达**——落在错误那页的读者必须能离开；
2. **冻结词原样保留**——`$1,000`、`ledger.db`、`expected-totals.json`、
   `start-ledgerbox.cmd`、`ledgerbox setup --client claude --data-dir`、`78%`、`SQLite`、`MCP`
   等；这些是读者要敲、机器要读的东西，不是词。另有一条反向检查断言这张清单里的每一项
   **仍然在英文页里**——一个已经离开英文页的词会让它那行检查变成空跑，而空跑读起来和通过一模一样；
3. **判定符号数目相等**——✅ 3、🔜 1、❌ 2。粗糙是故意的：它分不清哪一行该配哪个符号，
   但它让"翻译途中把一个叉悄悄升级成勾"这一种编辑做不到不被发现，而那正是本项目存在的理由；
4. **英文页指向的文档一个都不能丢**——威胁模型、安全策略、许可证、接入银行。
   一张想变短的页面最先想丢的就是这些不舒服的材料。

术语表钉住诚实性关键词：refused=拒收（不是"失败"），abstain=弃权（遗漏不是缺陷，猜测才是），
provenance=来源，withdraw a run=整轮撤回；并明写 `review_first`/`automatic`、
`transfer`/`cash`/`cash-deposit` 这类 wire 字面值与分类 ID **一律不翻**。
中文页还写明 `docs/` 目前全是英文——不假装不是。

顺序照旧：闸门先写，在没有译文时因"没有任何 `README.<lang>.md`，下面每条检查都是空跑"而红，
译文落地后转绿。

---

## 5bn. 中文适配第二阶段：字典层，缺键回落英文（2026-08-17）

`src/ledgerbox/web/js/i18n.js`。**英文原句既是键也是默认值**——这条选择就是整个设计。
`upload.dropzone.subtitle` 这样的键要开第二个文件才知道页面在说什么，会过期而不会报错，
还允许一句话在一处被改、在另一处被翻，直到两边说的不是一件事。用句子做键，英文页面按构造
就是事实源：**仓库里每一个钉英文字符串的既有测试一个字都没改**，因为未配置时 `t('Add statements')`
原样返回 `Add statements`。代价是改英文措辞会让译文成为孤儿——这是该付的代价：孤儿回落到英文，
是真的；过期的键继续显示旧句子，不是。

**什么都不猜。** 译文缺失、为空、不是字符串、或占位符与英文不一致时一律不用，显示英文并把该键
记进 `missingKeys()`。页面上出现 `{count} transactions` 或一个空按钮，比没翻译更糟。
反例 18 条覆盖：缺键、空串、非字符串、丢占位符、凭空多出占位符、没给值的占位符保留花括号
（打印 `undefined` 到一句关于钱的话里，正是本项目反对的形状）、选未注册语言留在英文、
注册两次是合并不是替换、静态扫描保留 markup 缩进、按整段规范化空白匹配、属性
（`aria-label`/`placeholder`/`title`）也翻而 `data-*` 不翻。

层是纯的：不碰 DOM、不碰 storage，所以 `node --test` 能在没有浏览器的情况下把它全部证伪。
`language.js` 才碰 `window`：读 localStorage、设 `<html lang>`、接语言下拉。存了一个这个构建
没有的语言不是错误也不是弹窗——留在英文（真的），下拉只列真有的语言。切换语言重新加载页面：
面板在 boot 时渲染一次，就地重跑每一个会是第二条渲染路径，也就是第二个"页面显示什么"的定义。

`locales/zh-CN.js` 覆盖 index.html 的静态骨架。**故意不翻的**：三段引用了其他模块仍以英文渲染的
标签（`(none)`、`0 transactions`）的解释性长文，以及被行内标签切碎、拼回来语序会坏的句子——
指着一个英文标签的中文解释，让读者去找一个找不到的东西，比留英文更糟。`missingKeys()` 会在
控制台把它们逐条列出来。

`tests/js/locales.test.js` 防的是一种很可能发生的失败：译文由 Agent 按 Skill 写出，
而找不到句子的 Agent 会很乐意造一个像样的。它断言字典里每个键都真的出现在 index.html 或某个
`t('...')` 调用里——试着塞一个 `'Export to QuickBooks'` 进去，它当场红。另外还断言每个
`locales/*.js` 都被 `all.js` 导入并注册（没人导入的字典与不存在无法区分）、每个键都真的答得出、
占位符集合一致。

同轮附 `ledgerbox-translate` Skill（Codex/Claude 两份，checked-in，**不进 wheel**——它要往
checkout 里写文件，装在别处无处可写）。它写死不翻项（分类 ID、wire 值、金额格式、命令、路径、
产品名）、给出术语表连同"每个词必须活下来的那点保留"、并要求 scope 表的 ✅/🔜/❌ 计数不变。
两条总纲写在最前：**翻不出来的句子留英文**（英文是真的，猜不是）；**永远不要软化一条限制**。

`index.html` 因这次改动一度到 401 行，撞上 400 行闸门。**没有放宽闸门**：把新增注释压到 4 行，
理由留在 `language.js` 里，回到 396 行。

**测试全绿之后，把页面真的打开了一次**（仓库外空白合成账本、临时端口、看完即停），
当场抓到三件测试抓不到的事：

1. **一个死键。** 启用 JS 时浏览器把 `<noscript>` 的内容整段当作**一个字符串字面量**（连
   `<p class="...">` 一起），所以扫描看到的"句子"是那一整段 markup，为它写的字典项永远匹配不上。
   更根本的理由比解析理由好：`<noscript>` 恰恰在 JavaScript 关掉时才显示，而这个模块就是
   JavaScript——从这里翻译它在原理上做不到。现在 `OPAQUE_TAGS` 跳过
   `noscript/script/style/template/code/pre`（`code` 里是被引用的字面值，不是文字），
   字典项删掉，并补上反例。
2. **静态扫过、JS 又改回英文。** 侧栏 `This ledger` 在 index.html 里被扫描翻成中文，
   随后 `agent-center.js` 重建侧栏时用英文字面量又写了回去——读者会看到它闪一下变回英文。
   这不是边界，是缺陷；该行现在走 `t()`。
3. **九个章节名根本不来自 markup。** 侧栏目录由 JS 渲染，index.html 里那份同名列表会被替换掉，
   所以只翻静态标记的话它们永远是英文。把渲染改走 `t()` 时 `agent-center.js` 到了 402 行，
   撞上 400 行闸门——**没有放宽闸门，做了那次拆分**：目录与徽章是"九个章节名怎么画"，
   剩下的是"本地 Agent 连没连上"，它们共用的是文件不是主题。新
   `sidebar-nav.js`（71 行）出生即带 `t()`，`agent-center.js` 落到 365 行。

（顺带记一条排查教训：改完模块后浏览器仍显示旧行为，不是代码没生效，是浏览器的模块缓存；
换端口起一个干净 server 才看到真结果。差一点就去"修"一个并不存在的 bug。）

`index.html` 的静态注释也因同一条闸门压过一次，396 行。

Node 从 70 涨到 **95**；Python 1122 passed / 100 skipped 不变——既有测试没有一个需要改，
这正是"英文原句即键"要买的东西。

---

## 5bo. 候选带上模板与同簇计数；顺带补上一个自己刚犯的指纹遗漏（2026-08-18）

`RELEASE_PLAN.md` §5。每个候选新增两个只读字段：`descriptor_template`（**本仓库自己的**
`descriptor_template()` 输出，也就是学习回路记规则的那个粒度）与 `occurrences`（**本次响应内**
同模板的行数，含自己）。价值直接：在这之前 Agent 要判断哪些行是同一个商户，只能拿那条它被要求
当作不可信数据的原始描述符自己猜；现在分组与"这个答案会教出什么规则"用同一套词汇。

边界写死在契约、参考模块和 dataclass 三处，措辞一致：**同模板意味着同一个交易对手，不意味着
同一件事**。退款与消费共用商户，方向在行上而不在模板里；`occurrences: 12` 落在一个只写着支付
机制词的描述符上，那是十二次弃权，不是一个大组。空模板谁也不代表，永远报 `occurrences: 1`。
计数只算这一页，不算整本账——按整本账报数等于告诉 Agent"有四行是同一个商户"却只给它两行，
而对没见过的钱提建议正是这条边界要挡的。

反例先行：既有那条钉死候选字段集合的用例**先红**（严出闸门本来就该红），随后逐条补上——
模板与 `descriptor_template()` 逐行相等、per-visit 数字变而模板不变时两行互相报 2、
全数字描述符模板为空且不成簇、`--limit 2` 时计数只描述返回的那一页。

**同轮抓到我自己在上一提交里犯的错。** `docs/AGENT_SETUP.md` 会被打包成 Skill 的
`references/agent-setup.md`，所以 §5bn 那次给它加一段说明，**悄悄改变了每个用户的出厂指纹**，
而指纹目录没滚动——任何一个原封未动的个人安装从那一刻起会被读成 `custom`，恰好卡死为它建的
非 force 升级路径。这正是 `_BEFORE_PASTE_SAFE_SETUP` 上方注释记录过一次的同一种事故，整套测试
全程绿着。现在补录 `_BEFORE_TRANSLATE_SKILL_NOTE`（经与 19c1b47 的真实 blob 逐文件核对，
只差 `agent-setup.md` 一项）与本轮的 `_BEFORE_CANDIDATE_TEMPLATE_FIELDS`。

**并且让它下次不可能悄悄发生**：目录只记录"已经被替换掉的"版本，所以它天然看不见"正在发生的
替换"。新增 `SHIPPED_BUNDLES` 绊线——把本版出厂的每个文件摘要钉在测试里，任何一个被打包文件
的改动都会让它红，而它的失败信息就是流程本身：先把**当前期望值**抄进新的 `_BEFORE_<改了什么>`、
加进 `PREVIOUS_OFFICIAL_BUNDLES`，**然后**才更新钉子。跳过前两步等于把所有既有安装变成 custom。
...
## 5br. 第一个陌生人式装机卡住了：目录又漏了一版，Skill 文本把出口焊死了（2026-08-19）

发布当天，产品负责人以新用户姿态在一个测试目录 clone 并说"帮我 set up 这个项目"。
两处卡住，各修各的因：

**其一，指纹目录的历史遗留。** 机器上有一份 2026-08-11 前后装的个人 claude-code Skill，
doctor 判 `custom`，setup 按设计拒绝覆盖、拒绝注册 MCP。取证（只读）：**drift 为零**——
每个文件与它自己的清单逐字节一致，是货真价实的官方旧版；它与 `_BEFORE_ONE_COMMAND_SETUP`
只差 `references/agent-setup.md` 一个文件，而那个文件的字节与 squash 前历史 `953c052`
（2026-08-10，fail-closed copied-setup 那版）的 `docs/AGENT_SETUP.md` **逐 blob 相同**——
用存档分支验的血缘，不是信安装树的自述。目录从来只记得有人记得的版本；`SHIPPED_BUNDLES`
绊线让此类遗漏从今往后在提交时变红，但绊线之前的老安装只能这样打捞：野外发现、对史验真、
补录 `_BEFORE_ONE_COMMAND_GUIDE`。反例先行：把打捞的指纹钉进测试，先红后绿；
修完本机 doctor 实测 `custom` → `outdated`，自动升级路径复通。**策略从头到尾不是缺陷**——
拒绝覆盖不认识的东西，正是挡在用户私改与静默替换之间的那道墙；缺陷在目录见识不够。

**其二，setup Skill 把死胡同写成了规则。** 原文"never pass `--force` or `--yes`"
一刀切，Agent 面对真 custom 只能停死，而 CLI 本来就有带预览、要确认的 force 路径。
产品负责人问"能不能自动更新、自动注册 MCP，或至少执行前询问而不是直接中断"。答案分三层：
未动过的官方旧版**本来就自动升级**（本次卡住纯因目录漏记）；MCP 注册**本来就自动**
（只因 install 失败被 fail-closed 拦下）；真正的 custom **不自动、但也不再停死**——
Skill 文本改为：跑 doctor、把差异摆给用户、明说"你的修改会丢"、拿到本次对话中的明确同意后
才跑 `agent install-skill --force --yes`，再重跑 setup 补 MCP；用户拒绝则原样退出。
总纲从"永不"改成"绝不凭自己判断加 `--force`/`--yes`，唯一例外在第 3 步、以用户明确同意为前提"。

**同轮的第二个 UX 决定：数据目录改为"带默认值的一问"。** 不放仓库里（`.gitignore` 挡不住
`git add -f`，而 `git clean -fdx` 会**删掉**被忽略的文件——一次例行的"重置 checkout"
就是全部财务记录的数据丢失；checkout 是可抛弃的，账本不是；CLI 守卫照旧拒绝仓库内路径）。
也不无声自建（几年财务数据的存放点必须是用户**自觉知道**、会去备份的位置）。
中间解：Skill 提议 `<checkout>-data` 同盘毗邻目录，"好"即完整回答，用户给别的路径则以别的为准，
答复前不创建任何东西。真实首跑里用户那端恰好自选了 `D:	est账本-data`——默认值与直觉重合的证据。

`ledgerbox-setup` Skill 不在 wheel 的 force-include 清单里，改它不动出厂指纹，无需滚目录。
...
## 5bt. 中文第三阶段第二批：十八个模块接完，页面自己交出了四件事（2026-08-20）

§5bs 修正过的那个数字（未接线模块里约 135 条英文散文，跨 18 个文件，另加 agent-center 余 16 条）
本轮全部接完，另加三条不在那张表里的：`charts.js` 的 `share not computable`、
`api.js` 的传输失败句、`main.js` 的状态条兜底句。六个提交，每个都自带失败反例。

**先修"同物两名"。** 侧栏目录走 `t()` 已经是中文，而它指向的三个面板把标题写成了英文字面量：
列表里写 `Agent 提案`，滚过去的那一节顶上写 `Agent proposals`。这比整页不翻更糟——
不翻的页面至少自己和自己一致，而这一页让读者去找一个不在上面的名字。
`tests/js/section-names.test.js` 用比较而不是期望值表来钉它：把每个小节的名字在英文下渲染一遍、
在该语言下再渲染一遍，谁的目录项动了而标题没动就红。修之前它一次点出全部三个。

### 四条这一轮才立起来的规矩

1. **服务端自己的句子照抄，本页补的那半句才翻。** 前半是本地进程在报告一个事实，
   后半是本页在说这个事实对你刚才那个动作意味着什么；一个键盖住两者，等于让词典
   为它并不拥有的措辞负责。proposal/triage 的 `failureCopy`、上传结果、表格上方那句
   bank-leg 汇总，全部按这条走。
2. **单复数写成两个完整句子，不是往句子里换一个名词。** 英语在 `line matches` 与
   `lines match` 之间挑；没有复数的语言两种情况说法相同，数词形态多于两种的语言需要更多。
   词典能回答一个句子，答不了半个。`transaction-status.js`、`1 round`/`{count} rounds`、
   `1 selected transaction.` 都是这么写的。
3. **两句之间那个空格永远在读取点，不在任何一句里面。** 键做空白规范化，而英文与其他语言
   走同一条查表路径，所以句首/句尾的空格会被从键里修掉、并跟着从页面上消失。
   `advice.js` 的 `net$1,234.56` 是第一例，`analytics.js` 三处、`transactions.js` 两处、
   `statements.js` 两处、`review.js`、`deletion-plan.js` 的破折号、`category-claim.js` 四处
   都是同一件事。`tests/js/advice-panel.test.js` 在修好之前正是红在那个字符串上。
4. **导入时建好的表在读取点查，不在写下的地方查。** `main.js` 先 import 全部模块、之后才选语言，
   所以 `PRESETS`、`KINDS`、`COLUMNS`、`IMPACT_ROWS`、`LOSSES` 里若存已翻译的字符串，
   拿到的会是英文，并且此后一辈子都是英文——而只读词典的检查全绿。
   `tests/js/late-language.test.js` 在选完语言之后再造控件，然后回头问词典哪些句子本该动；
   把快照缺陷模拟回去，它一次点出全部七个预设标签和两个筛选标签。

### 一个通不过的检查，被它自己要抓的缺陷抓到

`t('{count} line(s) match', { matched })` 把值放在了错的名字下。它读起来完全正确，
而它会把 `{count} line(s) match` 印在交易表上方——`i18n.js` 故意保留花括号而不是把
`undefined` 印进一句关于钱的话里，所以这个错是看得见的，但只对正在看那个面板那个状态的人看得见。
它是靠运气发现的。`tests/js/placeholders.test.js` 改为读调用点：一个小扫描器，
因为句子用 `+` 跨行拼、值对象里还嵌着自己的 `t()` 调用。
**它的第一版用了一个模块级的全局正则**，`matchAll` 会把 `lastIndex` 一起复制过去，
于是上一个测试里的 `test()` 让搜索从句子中间开始，漏掉了句首的占位符——
它对着自己被写出来要抓的那个缺陷跑绿了。每次现造一个正则，它点名 `transactions.js:185`。

### 字典撞上 400 行：闸门没有放宽

`locales/zh-CN.js` 涨到 446 行。词典是这里唯一一个会随页面每多一句话就变长的文件，
而 `registerLocale` 本来就是合并而不是替换（§5bn 的反例之一），所以语言按页面已有的缝分成
四个文件到达：外框、两个 Agent 审阅面板、其余面板、交易表。`locales/all.js` 列出各部分，
新增反例拒绝同一语言的两个文件都回答同一句话——一个问题两个答案，由 import 顺序决定谁赢。
`tests/js/locales.test.js` 的 tag 取第一个点之前那段（BCP-47 标签里没有点）。

`transactions.js` 也撞过一次 400，撞的是本轮自己新增的注释；**同样没有放宽闸门**，
压缩的是那些注释（§5bn 对 `index.html` 的同一个答案）。`chart-categories.js` 本来就正好 400 行，
所以图例那句行数计数被放进 `category-claim.js`——该文件的文件头本来就写着"图例的词汇在上一次
撞线时搬到了这里"。

### 顺带清掉的死文案

接线过程发现**七条从来没有任何读取点读过的字符串**：`advice.js` 两条、`date-range.js` 三条
（`index.html` 本来就带着那三个标签）、`transaction-bulk.js` 的 `selectPage`、
`chart-monthly.js` 的 `keyIn`/`keyOut`。没被读过的字符串正是会变成"永远不可能出现在页面上的
词条"的那个形状——一个跟着 Skill 干活的翻译 Agent 会老老实实为它写译文。删掉，不翻。

### 打开页面才说得出来的三件事

仓库外合成账本（18 行、3 个月、含转账/覆盖/大额）、临时端口、中英各读一遍、断服再读一遍。

1. **一条从来没走过 `t()` 的计数。** 类别图例的 `3 line(s)` 是模板字面量，
   所以它在中文页上是英文，**而且 `missingKeys()` 看不见它**——一份关于缺口的报告，
   报不了一句从来没被问过的话。
2. **散文里的"同物两名"。** 两句话让读者"到 Transactions 里改"，而这一版把那个小节
   标成也列成 `交易`。`section-names.test.js` 比对的是标题与目录项，看不见段落里的名字。
3. **一条说三、实际是四的注释。** `zh-CN.js` 列了它有意留英文的段落；逐月图那段导语是第四个，
   被行内 `<em>` 切开，和另外三个被 `<strong>` 切开是同一回事。
   全译页面上 `missingKeys()` 报 **27** 条，每一条都属于这四段、属于两个故意保持各自语言的
   语言选项、属于产品名，或者是一个破折号。

断服状态下六个面板都说同一句 `正在等待 ledgerbox。`，理由只在连接灯下说一次，
侧栏说 `无法获知 Agent 状态`——与 §5bp 定下的形状一致。

Node 从 100 涨到 **108**；Python 1138 passed / 100 skipped 不变——**既有测试一条都没有改**，
这正是"英文原句即键"要买的东西。合成账本与临时 launch 配置用完即删。
