...
3. Run the setup command with their answer and this client:
   ...
   ```powershell
   .venv\Scripts\ledgerbox.exe setup --client claude --data-dir <their folder>
   ```
   ...
   silently. Run `ledgerbox agent doctor --client claude`, show the user
   ...
   ```powershell
   .venv\Scripts\ledgerbox.exe agent install-skill --client claude --force --yes
   ```
